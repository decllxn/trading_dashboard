...
import { getMarketData, fetchAlpacaBars, fetchTwelveDataBars } from './market-data.ts';
import { db } from '../db/index.ts';
import { marketDataCache } from '../db/schema.ts';
import { eq, and } from 'drizzle-orm';

// We only run these tests if the credentials are present in the environment
...
test('Alpaca client fetches SPY daily bars', { skip: !hasAlpaca }, async () => {
  console.log('Fetching SPY bars from Alpaca...');
  const bars = await fetchAlpacaBars('SPY', '1D', '2024-01-01');
...
test('Twelve Data client fetches EUR/USD daily bars', { skip: !hasTwelveData }, async () => {
  ...
  // EUR/USD close on 2024-01-02 is around 1.094
  assert.ok(fxBar.value > 1.05 && fxBar.value < 1.15, 'EUR/USD rate should be within standard historical range');
});

test('Caching layer stores and retrieves cached responses', { skip: !db || !hasAlpaca }, async () => {
  ...
  await db.delete(marketDataCache).where(
    and(
      ...
      eq(marketDataCache.timeframe, '1D')
    )
  );

  console.log('First call to getMarketData (should miss cache & fetch)...');
  const data1 = await getMarketData(testSymbol, '1D', '2024-01-01');
  ...
  const data2 = await getMarketData(testSymbol, '1D', '2024-01-01');
