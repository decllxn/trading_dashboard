...
export function BestTradeForm({ tradesList, onSuccess, onCancel }: BestTradeFormProps) {
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <div className="sticky bottom-0 z-20 -mx-4 sm:-mx-6 -mb-4 sm:-mb-6 mt-6 border-t border-hairline bg-surface p-4 sm:px-6 flex items-center justify-end gap-3 shrink-0">
        {onCancel && (
          <button
            type="button"
            onClick={onCancel}
            className="border border-hairline text-secondary hover:text-primary rounded-card px-4 py-2 text-xs font-medium min-h-[44px] transition-colors"
          >
            Cancel
          </button>
        )}
        <SubmitButton pendingLabel="Saving..." className="min-h-[44px]">Save Best Trade</SubmitButton>
      </div>
    </form>
  );
}
